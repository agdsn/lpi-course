...
%   \end{block}
  \begin{block}{Wichtige Dateien}
    \begin{description}
      \item[\co{/dev/fstab}] Zuordnung bekannter Datenträger mit Einhängepunkten
      \item[\co{/dev/mtab}] Aktuell eingebundene Datenträger
      \item[\co{/dev/inittab}] Systemstart
      \item[\co{/dev/init.d/}] Skripte für den Systemstart
      \item[\co{/dev/hosts}] Zuordnung von Rechner und Namen im kleinen Netzen
      \item[\co{/dev/passwd}] Benutzerdaten (Namen, Gruppen, \dots)
      \item[\co{/dev/issue}] Text, der vor einer Anmeldung angezeigt wird
      \item[\co{/dev/motd}] Nachrichtentext nach der Ameldung
    \end{description}
  \end{block}
\end{frame}
